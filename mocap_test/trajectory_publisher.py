...
Trajectory (5 legs, all body-frame relative to the virtual centre):
    ...
    A:  1 --> 2   strafe forward 1.0 m
    B:  2 --> 3   arc CCW 180 deg, radius 0.5 m
    C:  3 --> 4   strafe forward 1.0 m
    D:  4 --> 5   pivot +90 deg (rotation only)
    E:  5 --> 1   strafe forward 1.0 m   (closes the loop)

World-frame layout (start at origin facing world +Y / "north"):

    1 = (0,  0)  heading  90 deg
    2 = (0,  1)  heading  90 deg
    3 = (-1, 1)  heading 270 deg   (after CCW arc, R=0.5)
    4 = (-1, 0)  heading 270 deg
    5 = (-1, 0)  heading   0 deg   (after +90 deg pivot)
...
# ── Trajectory geometry ──────────────────────────────────────────────
LEG_LENGTH_M  = 1.0     # straight legs A, C, E
ARC_RADIUS_M  = 0.5     # arc B
PIVOT_ANGLE_RAD = math.pi / 2.0   # leg D

# ── Default speeds (body-frame, virtual centre) ──────────────────────
# Chosen so the outer robot of a 0.9 m formation stays under 0.18 m/s
# during the arc:
#   v_robot_center  = vx + omega * (D/2)       (worst-case offset)
#   v_wheel_max     = v_robot_center + omega * 0.212   (chassis half-diag)
#   For D=0.9, vx=0.07, omega=0.15:
#       v_robot_center = 0.07 + 0.0675 = 0.1375 m/s
#       v_wheel_max    = 0.1375 + 0.0318 = 0.1693 m/s   ✓ under 0.18
DEFAULT_STRAFE_VX   = 0.10    # legs A, C, E (no rotation, no constraint)
DEFAULT_ARC_VX      = 0.07    # leg B (combined with omega)
DEFAULT_ARC_OMEGA   = 0.15    # leg B (=> arc duration = pi/0.15 ≈ 20.94 s)
DEFAULT_PIVOT_OMEGA = 0.20    # leg D (no translation)

# Pause between legs so checkpoints are visible in the bag as flat
# zero-velocity sections. 2 s is enough to see a clean stop on the
# odom/MoCap traces.
CHECKPOINT_PAUSE_S = 2.0
